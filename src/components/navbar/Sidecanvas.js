    ...
    return (
        <>
                    ...
                    <Container>
                        <div className='box d-flex flex-row justify-content-between gap-1'>
                            ...
                            <div className='Userdetails  fw-bold '>
                                <h6>User : {userDetails?.userid ? userDetails.userid : ' __'}</h6>
                                <ul className='text-info'>
                                    <li> Name : <p className='text-success'>{userDetails?.Username ? userDetails.Username : ' --'}</p>  </li>
                                    <li> DoB :<p className='text-success'> {userDetails?.dateOfBirth ? userDetails.dateOfBirth : " --"}</p></li>
                                    <li> E-mail :<p className='text-success'> {userDetails?.E_mail ? userDetails.E_mail : " --"}</p></li>
                                </ul>
                            </div>
                        </div>
                        ...
                        <div>
                            <h5 className='fw-bold text-danger '>Public</h5>
                            <ul className='public-li'>
            ...
            <MyVerticallyCenteredModal prop={modalShow.toString()}
                show={modalShow}
                onHide={() => setModalShow(JSON.parse('false'))}
                handleclose={ () => setShow(JSON.parse('false'))}
            />
        </>
    );
}
